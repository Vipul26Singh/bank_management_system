...
<!--Start Panel-->
<div class="alert alert-info"><p>  Deduct fund from the account</p></div>
<div class="panel panel-default">
    <!-- Default panel contents -->
    <div class="panel-heading"><?php get_phrase('debit_account') ?></div>
 ...
 <div class="form-group">
        <label for=""><?php get_phrase('debit_account_type') ?><span style="color:red">  *</span></label>
                <select class="form-control" id='debit_account_type' name='debit_account_type'>
                    <option value=''>Select</option>

			<?php
                                foreach($debit_account_type as $account){
 ...
 <div class="form-group">
  ...
  <div class="form-group">
        <label for="debit_mode"><?php get_phrase('debit_mode') ?><span style="color:red">  *</span></label>
                <select class="form-control" id='debit_payment_mode' name='debit_payment_mode'>
                    <option value=''>Select</option>

                        <?php
                                foreach($available_debit_mode as $debit_mode){


                        ?>
                        <option value='<?php echo json_encode(array("extra_detail"=>$debit_mode["extra_detail"], "value"=>$debit_mode["value"])) ?>'><?php echo $debit_mode['name']?></option>
                        <?php
                                }
                        ?>
            </select>
 ...
 <div id="extra_detail_div" hidden>
	<div class="form-group">
		...
		<input type="text" class="form-control" name="debit_payment_number" id="debit_payment_number" placeholder="Cheque/DD/etc no">
	</div>

	<div class="form-group">
                <label for="bank_ifsc"><?php get_phrase('bank_ifsc') ?><span style='color:red'>  *</span></label></label>
                <input type="text" class="form-control" name="debit_payment_bank_ifsc" id="debit_payment_bank_ifsc" placeholder="Bank ifsc code">
        </div>

	<div class="form-group">
                <label for="bank_name"><?php get_phrase('bank_name') ?></label>
                <input type="text" class="form-control" name="debit_payment_bank_name" id="debit_payment_bank_name" placeholder="Name of the bank">
        </div>

	<div class="form-group">
                <label for="bank_branch"><?php get_phrase('bank_branch') ?></label>
                <input type="text" class="form-control" name="debit_payment_bank_branch" id="debit_payment_bank_branch" placeholder="Branch of the bank">
        </div>	
 </div>

 <div class="form-group">
    <label for="account_balance"><?php get_phrase('debit_amount') ?><span style='color:red'>  *</span></label>
    <input type="number" class="form-control" name="debit_amount" id="debit_amount" placeholder=<?php get_phrase('debit_amount') ?>>
  </div>
...
$(document).ready(function(){
                ...
                $("#debit_payment_mode").change(function(){
                        var payment_detail = JSON.parse($("#debit_payment_mode").val());
...
<script type="text/javascript">
$(document).ready(function(){
                $("#debit_account_type").change(function(){
			$('#account_number').val('');	
			$('#member_id').val('');       
                        $('#member_name').val('');
                        $('#member_age').val('');
...
$(document).ready(function(){
	$('#account_number').change('input',function(e){
			...
			var account_type = $("#debit_account_type").val();
			$('#member_id').val('');       
                        $('#member_name').val('');
			$('#member_age').val('');
			$('#account_balance').val('');
...
$(document).ready(function(){
	$('#add-debit').on('submit',function(){
		$.ajax({
			method : "POST",
			url : "<?php echo site_url('Admin/debitAccount/insert') ?>",
			data : $(this).serialize(),
			beforeSend : function(){
