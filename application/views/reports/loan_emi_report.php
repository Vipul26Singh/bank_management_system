...
<form id="search" action="<?php echo base_url()?>Reports/loan_emi_report/search" method="get">
	    ...
	    <td><?php echo $account->member_name ?></td>
         <td><?php echo $account->account_balance ?></td>
          <td><?php if($account->bank_account_type=='1'){echo 'Current';}else{echo 'Savings';}?></td>
           ...
           <td><a href="<?php echo site_url('Reports/loan_emi_report/view?id='.$account->member_id)?>" class="mybtn btn-info btn-xs account-edit-btn"> <?php get_phrase('view') ?></a></td>
