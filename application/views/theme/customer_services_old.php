...
<!--Start Panel-->
<div class="alert alert-info"><p>Add Customer Service Here</p><p>Example:- SMS, MAINTAINENCE</p></div>
<div class="panel panel-default">
 ...
 <div class="form-group"> 
    <label for="account"><?php get_phrase('service_name') ?></label>
    <input type="text" class="form-control" maxlength="30" name="service_name" id="service_name"/>   
  </div> 
        
  <div class="form-group"> 
    <label for="p-type"><?php get_phrase('service_amount') ?></label>
...
<div class="panel panel-default">
    ...
    <div class="panel-body">
       <table class="scroll-table-head table table-striped table-bordered table-condensed">
        <th class="sc-col-4"><?php get_phrase('service_name') ?></th>
    <th class="sc-col-3"><?php get_phrase('service_amount') ?></th>
    <th class="sc-col-3"><?php get_phrase('action') ?></th>
       ...
       <?php foreach($services as $list){ ?>
        ...
        <tr>
        <td class="sc-col-4 t_name"><?php echo $list->service_name ?></td>
        <td class="sc-col-3 t_type"><?php echo $list->service_amount ?></td>  
        <td class="sc-col-3"><a class="mybtn btn-info btn-xs payee-edit-btn" href="<?php echo $list->tbl_id?>"><?php get_phrase('edit') ?></a>
        <a class="mybtn btn-danger btn-xs payee-remove-btn"  href="<?php echo  site_url('Admin/customerServices/remove/'. $list->tbl_id) ?>"><?php get_phrase('remove') ?></a></td>   
        </tr>
...
$.ajax({
method : "POST",
url : "<?php echo site_url('Admin/customerServices/insert') ?>",
data : $(this).serialize(),
beforeSend : function(){
...
$("#service_name").val($(main).closest("tr").find(".t_name").html()); 
$("#service_amount").val($(main).closest("tr").find(".t_type").html());  
//get table index
...
function updateRow(){
var an=$("#service_name").val();
var at=$("#service_amount").val();
