...
<form id="search" action="<?php echo base_url()?>Admin/item_management/search" method="get">
  ...
  <input type="search" name="search_name" id="search_name" placeholder="Search by Name">
    ...
    <div class="panel-body manage-client">
		...
		<th><?php get_phrase('member_id') ?></th>
         ...
         <th><?php get_phrase('manage_service')?></th>
	    ...
	    <td><?php echo $account->member_name ?></td>
         <td><?php echo $account->account_balance ?></td>
          <td><?php if($account->bank_account_type=='1'){echo 'Current';}else{echo 'Savings';}?></td>
           <td><?php if(!empty($account->opening_date)){echo $account->opening_date; }?></td>
           <td><a href="<?php echo site_url('Admin/item_management/manage?id='.$account->member_id)?>" class="mybtn btn-info btn-xs account-edit-btn"> <?php get_phrase('manage') ?></a></td>
