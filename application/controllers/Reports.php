...
class Reports extends CI_Controller {
        ...
        public function datewiseIncomeReport($action='')
    {
        ...
        echo "<tr><td colspan='3'><b>Total Income</b></td>";
        ...
        public function datewiseExpenseReport($action='')
    {
      ...
      <td class="text-right"><?php echo get_current_setting('currency_code')." ".$report->amount ?></td>
        ...
        echo "<tr><td colspan='2'><b>Total Expense</b></td>";
        echo "<td class='text-right'><b>".get_current_setting('currency_code')." ".$sum."</b></td>";
        echo "<td>Sub Total</td></tr>";     
        }
      }
        ...
        public function transferReport($action='')
    {
        ...
        <?php 
         ...
         echo "<tr><td colspan='3'><b>Total</b>";
         echo "<td class='text-right'><b>".get_current_setting('currency_code')." ".$dr."</b></td>";
    ...
    public function reportByPayee($action=''){
        ...
        public function loan_report($action='')
        {

        if($action=='asyn'){
        $this->load->view('reports/loan-report');
        }else if($action==''){
        $this->load->view('theme/include/header');
        $this->load->view('reports/loan-report');
        $this->load->view('theme/include/footer');
        ...
        $from_date=$this->input->post('from-date',true);
        $to_date=$this->input->post('to-date',true); 
        $reportData=$this->Reportmodel->getLoan($from_date,$to_date);
        if(empty($reportData)){
        echo "false";
        }else{
        foreach ($reportData as $report) { 
        ...
        <tr><td><?php echo $report->loan_id ?></td>
        <td><?php echo $report->member_id ?></td>
        <td><?php echo $report->loan_amount ?></td>
        <td><?php echo $report->year ?></td>
        <td><?php echo $report->interest_rate ?></td>
        <td><?php echo $report->total_loan_amount_with_interest ?></td>
        <td><?php echo $report->introducer_1_id ?></td>
        <td><?php echo $report->introducer_2_id ?></td>
        <td><?php echo $report->date ?></td>
        <td><?php echo $report->active_by ?></td>
        ...
        public function fixed_deposit_report($action='')
        {
        $data = array();
        if($action=='asyn'){
        $this->load->view('reports/fixed-d-report');
        }else if($action==''){
        $this->load->view('theme/include/header');
        $this->load->view('reports/fixed-d-report');
        $this->load->view('theme/include/footer');
        ...
        $from_date=$this->input->post('from-date',true);
        $to_date=$this->input->post('to-date',true); 
        $reportData=$this->Reportmodel->getFd($from_date,$to_date);
        if(empty($reportData)){
        echo "false";
        }else{
        foreach ($reportData as $report) { 
        ...
        <tr><td><?php echo $report->fd_id ?></td>
        <td><?php echo $report->member_id ?></td>
        <td><?php echo $report->interest ?></td>
        <td><?php echo $report->duration ?></td>
        <td><?php echo $report->fixed_deposit_amount ?></td>
        <td><?php echo $report->maturity_amount ?></td>
        <td><?php echo $report->date ?></td>
        <td><?php echo $report->maturity_date ?></td>
        <td><?php echo $report->taken ?></td>
        <td><?php echo $report->active_by ?></td>
        </tr>
        ...
        public function share_accounts($action='')
        {
        $data = array();
        if($action=='asyn'){
        $this->load->view('reports/share_accounts');
        }else if($action==''){
        $this->load->view('theme/include/header');
        $this->load->view('reports/share_accounts');
        $this->load->view('theme/include/footer');
        ...
        $from_date=$this->input->post('from-date',true);
        $to_date=$this->input->post('to-date',true); 
        $reportData =$this->Reportmodel->getShare($from_date,$to_date);
        if(empty($reportData)){
        echo "false";
        }else{
        foreach ($reportData as $report) { 
        ...
        <tr><td><?php echo $report->share_id ?></td>
        <td><?php echo $report->member_id ?></td>
        <td><?php echo $report->share_amount ?></td>
        <td><?php echo $report->date ?></td>
        </tr>
        ...
        public function loan_emi_report($action=''){
        $data=array();  
        $data['accounts']=$this->Reportmodel->getAllAccounts();
        if($action=='asyn'){  
            $this->load->view('reports/loan_emi_report',$data);
        }else if($action==''){
            $this->load->view('theme/include/header');
            $this->load->view('reports/loan_emi_report',$data);
            $this->load->view('theme/include/footer');
            ...
            $reportData['reportData'] = $this->Reportmodel->get_loan_emi($id);
             if(empty($reportData)){
        echo "false";
        }else{
       $this->load->view('theme/include/header');
            $this->load->view('reports/view_loan_emi',$reportData);
            $this->load->view('theme/include/footer');
    ...
    public function user_transaction_report($action=''){
           $data['accounts']=$this->Adminmodel->getAccount();
        
         if($action=='asyn'){  
            $this->load->view('reports/user_10_transactions',$data);
        }else if($action==''){
            $this->load->view('theme/include/header');
            $this->load->view('reports/user_10_transactions',$data);
            $this->load->view('theme/include/footer');
       ...
       $member_id = $this->input->get('id');
                   ...
                   $this->db->limit(10);
                  $query = $this->db->get();
                  $reportData['reportData'] = $query->result();
                 
			$this->load->view('theme/include/header');
			$this->load->view('reports/view_user_reports',$reportData);
			$this->load->view('theme/include/footer');
